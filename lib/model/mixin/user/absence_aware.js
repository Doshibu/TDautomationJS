...
module.exports = function (sequelize) {
  ...
  this.promise_calendar = function (args) {
    ...
    return Promise.join(
      ...
      Promise.try(function () {
        return this_user.promise_schedule_I_obey()
      }),
  ...
  this._calculate_total_number_of_days_n_allowance = function (args) {
    let
      self = this,
      year = args.year,
    ...
    return self.department.allowance +
      self.get_automatic_adjustment() +
      carried_over_allowance +
      adjustment
  }

  this.promise_total_number_of_days_in_allowance = function (year) {
    let self = this
  ...
  this.validate_leave_fits_into_remaining_allowance = function (args) {
    ...
    .then(employee =>
      employee.promise_number_of_days_available_in_allowance(
        year.format('YYYY')
      )
      .then(number_of_days => Promise.resolve([number_of_days, employee]))
    )
    .then(function (args) {
      ...
      if (
        ...
        console.log('Days remaining: ' + days_remaining_in_allowance + ', dayes deducted: ' + deducted_days)
