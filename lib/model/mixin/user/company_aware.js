...
module.exports = function (sequelize) {
  ...
  this.get_company_for_export = function () {
    return this.getCompany({
      include: [{
        model: sequelize.models.LeaveType,
        as: 'leave_types'
        ...
        model: sequelize.models.User,
        as: 'users',
        include: [{model: sequelize.models.Leave, as: 'my_leaves'}]
      }]
    })
    .then(function (company) {
      // Erase password hash record as it is pottencially possible to reverse engineer
      // the installatioon 'salt'
      company.users.forEach(function (user) { user.password = '' })
