...
router.get('/edit/:user_id/absences/', function (req, res) {
  ...
  .then(employee => Promise.join(
    ...
    (args, total_days_number, employee_adjustment, carried_over_allowance) => {
...
router.post('/edit/:user_id/', function (req, res) {
  ...
  // withing system
  .then(function () {
    return ensure_email_is_not_used_elsewhere({
      employee: employee,
  ...
  .then(function () {
    return ensure_user_was_not_useed_elsewhere_while_being_inactive({
      employee: employee,
      new_user_attributes: new_user_attributes,
      req: req,
      model: model
  ...
  .then(function () {
    return ensure_we_are_not_removing_last_admin({
      employee: employee,
      new_user_attributes: new_user_attributes,
      req: req,
      model: model
